...
class PlayRedPacketAction extends Action {
    ...
    public function index(){
        ...
        $redpacket_id = I('get.id');//获取红包游戏id(不是由第三方活动跳转有该参数)

        if($redpacket_id==""){
            exit;
        }

        $userInfo = getUserInfo(U('PlayRedPacket/index',array('id'=>$redpacket_id)));
        ...
        if($nowTime>=$redpacket['start_time'] && $nowTime<=$redpacket['end_time']){
            ...
            if(!$userLog){
                ...
                if($userLog['prize_id']==0){
                    $canPlayInfo = "你已经参加过<br/>每个微信号只允许玩一次";
                }else if($userLog['prize_id']>0 && ($userLog['name'] =='' || $userLog['name'] ==null)){
                    $canPlayInfo = "你已经参加过<br/>每个微信号只允许玩一次";
                }else if($userLog['prize_id']>0 && $userLog['name'] !='' && $userLog['name'] !=null){
    ...
    public function userInfo(){
        ...
        if(IS_POST){
            ...
            $userLog['phone_type'] = I('post.phone_type');
